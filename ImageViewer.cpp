...
void ImageViewer::activeFunction(){
    ...
    featureSiftActWithMask->setEnabled(true);
    ...
    objectRemovingAct->setEnabled(true);
...
void ImageViewer::about()
{
    QMessageBox::about(this, tr("About Image Processing"),
                       tr("<p>The <b>Image Processing</b> shows how to process digital images.</p>"));
}
...
void ImageViewer::createActions()
{
    ...
    zoomInAct = new QAction(QIcon(":/images/1uparrow.png"),tr("Zoom &In (25%)"), this);
    zoomInAct->setShortcut(tr("Ctrl++"));
    zoomInAct->setEnabled(false);
    connect(zoomInAct, SIGNAL(triggered()), this, SLOT(zoomIn()));

    zoomOutAct = new QAction(QIcon(":/images/1downarrow.png"),tr("Zoom &Out (25%)"), this);
    ...
    connect(gaussianBlurAct, SIGNAL(triggered()), this, SLOT(gaussianBlurAction()));
    ...
    connect(bilateralFilterAct, SIGNAL(triggered()), this, SLOT(bilateralFilterAction()));

    convolutionFilterAct = new QAction(tr("Convolution filter"), this);
    convolutionFilterAct->setEnabled(false);
    connect(convolutionFilterAct, SIGNAL(triggered()), this, SLOT(convolutionFilterAction()));
    ...
    connect(findContourAct, SIGNAL(triggered()), this, SLOT(findContourAction()));
    ...
    connect(houghCircleAct, SIGNAL(triggered()), this, SLOT(houghCircleAction()));
    ...
    connect(bdBoxCircleAct, SIGNAL(triggered()), this, SLOT(bdBoxCircleAction()));

    bdRotEclipseAct = new QAction(tr("Bounding Rotate and Eclipse"), this);
    bdRotEclipseAct->setEnabled(false);
    connect(bdRotEclipseAct, SIGNAL(triggered()), this, SLOT(bdRotEclipseAction()));
    ...
    connect(cornerDetectAct, SIGNAL(triggered()), this, SLOT(cornerDetectAction()));

    featureSurfAct = new QAction(tr("SUFT feature detector"), this);
    featureSurfAct->setEnabled(false);
    connect(featureSurfAct, SIGNAL(triggered()), this, SLOT(featureSurfAction()));

    featureSiftAct = new QAction(tr("SIFT feature detector"), this);
    featureSiftAct->setEnabled(false);
    connect(featureSiftAct, SIGNAL(triggered()), this, SLOT(featureSiftAction()));

    featureSiftActWithMask = new QAction(tr("SIFT feature detector with mask"), this);
    featureSiftActWithMask->setEnabled(false);
    connect(featureSiftActWithMask, SIGNAL(triggered()), this, SLOT(featureSiftActionWithMask()));

    matchingBFAct = new QAction(tr("Description matching with Brute Force"), this);
    ...
    connect(matchingFLANNAct, SIGNAL(triggered()), this, SLOT(matchingFLANNAction()));

    matchingHomographyAct = new QAction(tr("Description matching Homography"), this);
    matchingHomographyAct->setEnabled(false);
    connect(matchingHomographyAct, SIGNAL(triggered()), this, SLOT(matchingHomographyAction()));

    matchingTemplateAct = new QAction(tr("Matching template"), this);
    matchingTemplateAct->setEnabled(false);
    connect(matchingTemplateAct, SIGNAL(triggered()), this, SLOT(matchingTemplateAction()));

    correctMorphAct = new QAction(tr("Correct now"), this);
    correctMorphAct->setEnabled(false);
    connect(correctMorphAct, SIGNAL(triggered()), this, SLOT(correctMorphAction()));
    ...
    connect(growingMyImplAct, SIGNAL(triggered()), this, SLOT(growingMyImplAction()));
    ...
    //remove object
    objectRemovingAct = new QAction(tr("Remove object"), this);
    objectRemovingAct->setEnabled(false);
    connect(objectRemovingAct, SIGNAL(triggered()), this, SLOT(objectRemovingAction()));
...
void ImageViewer::createMenus()
{
    ...
    mnuFeaDetector->addAction(featureSiftActWithMask);
    mnuFeaDetector->addSeparator();
    ...
    //add menu object removing
    myimplMenu->addAction(objectRemovingAct);
...
void ImageViewer::contextMenuEvent(QContextMenuEvent *event) // this is a slot
{
    ...
    if (parameterPanel!=NULL) menu.addAction(parameterAct);
...
bool ImageViewer::save(){
    QString fileName = QFileDialog::getSaveFileName(this,
                                                    tr("Save image"), ".", tr("Image Files (*.jpg)"));
    if (fileName.isEmpty()){
        return false;
    }
...
void ImageViewer::normalBlurAction(){
    ...
    ImageViewer* other = new ImageViewer;
    other->loadImage(matImage, ImageConvert::cvMatToQImage(dstMat), "normal blur result");
    other->addParameterPanel(new ui::NormalBlurPanel(other),x() + 40, y() + 40);
}

void ImageViewer::normalBlurAction(int kernelSize){
    cv::Mat dstMat;
    cv::blur(matImage, dstMat, cv::Size(kernelSize, kernelSize), cv::Point(-1, -1));
...
void ImageViewer::gaussianBlurAction(){
    ...
    ImageViewer* other = new ImageViewer;
    other->loadImage(matImage, ImageConvert::cvMatToQImage(dstMat), "gaussian blur result");
    other->addParameterPanel(new ui::GaussianBlurPanel(other),x() + 40, y() + 40);
}
...
void ImageViewer::medianBlurAction(){
    ...
    ImageViewer* other = new ImageViewer;
    other->loadImage(matImage, ImageConvert::cvMatToQImage(dstMat), "median blur result");
    other->addParameterPanel(new ui::MedianBlurPanel(other),x() + 40, y() + 40);
...
void ImageViewer::bilateralFilterAction(){
    ...
    ImageViewer* other = new ImageViewer;
    other->loadImage(matImage, ImageConvert::cvMatToQImage(dstMat), "bilateral filter result");
    other->addParameterPanel(new ui::BilateralFilterPanel(other),x() + 40, y() + 40);
}

void ImageViewer::bilateralFilterAction(int sigmaColor){
    cv::Mat dstMat;
    cv::bilateralFilter( matImage, dstMat, sigmaColor, sigmaColor*2, sigmaColor/2 );
...
void ImageViewer::convolutionFilterAction(){
    ...
    /// Apply filter
    cv::filter2D(matImage, dst, ddepth , kernel, anchor, delta, cv::BORDER_DEFAULT );

    ImageViewer* other = new ImageViewer;
    other->loadImage(matImage, ImageConvert::cvMatToQImage(dst), "convolution filter result");
    other->addParameterPanel(new ui::ConvolutionFilterPanel(other),x() + 40, y() + 40);
...
void ImageViewer::erosionAction(){
    ...
    ImageViewer* other = new ImageViewer;
    other->loadImage(matImage, ImageConvert::cvMatToQImage(dstMat), "erosion filter result");
...
void ImageViewer::dilationAction(){
    ...
    ImageViewer* other = new ImageViewer;
    other->loadImage(matImage, ImageConvert::cvMatToQImage(dstMat), "dilation filter result");
...
void ImageViewer::morphoExtAction(){
    ...
    ImageViewer* other = new ImageViewer;
    other->loadImage(matImage, ImageConvert::cvMatToQImage(dstMat), "morphology ext. result");
...
void ImageViewer::thresholdAction(){
    ...
    ImageViewer* other = new ImageViewer;
    other->loadImage(src_gray, ImageConvert::cvMatToQImage(dst), "thresholding result");
    other->addParameterPanel(new ui::ThresholdingPanel(other),x() + 40, y() + 40);
}
...
void ImageViewer::thresholdAction(int value, int type){
    ...
    qImage = ImageConvert::cvMatToQImage(dstMat);
    imageLabel->setPixmap(QPixmap::fromImage(qImage));
    statusBar()->showMessage(tr("Value: ") + QString::number(value)
                             + tr("| Type: ") + QString::number(type));
}
...
void ImageViewer::sobelAction(){
    ...
    cv::GaussianBlur( matImage, matImage, cv::Size(3,3), 0, 0, cv::BORDER_DEFAULT );
    ...
    ImageViewer* other = new ImageViewer;
    other->loadImage(src_gray, ImageConvert::cvMatToQImage(grad), "sobel result");
...
void ImageViewer::sobelAction(int kernelSize, int scale){
    ...
    /// Gradient X
    cv::Sobel( matImage, grad_x, ddepth, 1, 0, kernelSize, scale, delta, cv::BORDER_DEFAULT );
    cv::convertScaleAbs( grad_x, abs_grad_x );

    /// Gradient Y
    cv::Sobel( matImage, grad_y, ddepth, 0, 1, kernelSize, scale, delta, cv::BORDER_DEFAULT );
    cv::convertScaleAbs( grad_y, abs_grad_y );
...
void ImageViewer::scharrAction(){
    ...
    ImageViewer* other = new ImageViewer;
    other->loadImage(src_gray, ImageConvert::cvMatToQImage(grad), "scharr result");
...
void ImageViewer::scharrAction(int scale){
    ...
    /// Gradient X
    cv::Scharr( matImage, grad_x, ddepth, 1, 0, scale, delta, cv::BORDER_DEFAULT );
    cv::convertScaleAbs( grad_x, abs_grad_x );

    /// Gradient Y
    cv::Scharr( matImage, grad_y, ddepth, 0, 1, scale, delta, cv::BORDER_DEFAULT );
    cv::convertScaleAbs( grad_y, abs_grad_y );
...
void ImageViewer::laplaceAction(){
    ...
    ImageViewer* other = new ImageViewer;
    other->loadImage(src_gray, ImageConvert::cvMatToQImage(abs_dst), "laplace result");
...
void ImageViewer::laplaceAction(int kernelSize, int scale){
    ...
    cv::Laplacian( matImage, dst, ddepth, kernelSize, scale, delta, cv::BORDER_DEFAULT );
    cv::convertScaleAbs( dst, abs_dst );
...
void ImageViewer::cannyAction(){
    ...
    ImageViewer* other = new ImageViewer;
    other->loadImage(matImage, ImageConvert::cvMatToQImage(dst), "canny result");
...
void ImageViewer::houghLineAction(){
    ...
    ImageViewer* other = new ImageViewer;
    other->loadImage(matImage, ImageConvert::cvMatToQImage(cdst), "hough line result");
    other->move(x() + 40, y() + 40);
    other->show();
...
void ImageViewer::houghCircleAction(){
    ...
    ImageViewer* other = new ImageViewer;
    other->loadImage(matImage, ImageConvert::cvMatToQImage(matImage), "hough circle result");
    other->move(x() + 40, y() + 40);
    other->show();
}
...
void ImageViewer::findContourAction(){
    ...
    ImageViewer* other = new ImageViewer;
    other->loadImage(matImage, ImageConvert::cvMatToQImage(drawing), "finding contour result");
    other->move(x() + 40, y() + 40);
    other->show();
...
void ImageViewer::convexHullAction(){
    ...
    ImageViewer* other = new ImageViewer;
    other->loadImage(matImage, ImageConvert::cvMatToQImage(drawing), "convex hull result");
    other->move(x() + 40, y() + 40);
    other->show();
...
void ImageViewer::bdBoxCircleAction(){
    ...
    ImageViewer* other = new ImageViewer;
    other->loadImage(matImage, ImageConvert::cvMatToQImage(drawing), "bounding and circle result");
    other->move(x() + 40, y() + 40);
    other->show();
...
void ImageViewer::bdRotEclipseAction(){
    ...
    ImageViewer* other = new ImageViewer;
    other->loadImage(matImage, ImageConvert::cvMatToQImage(drawing), "bounding rotate and eclipse result");
    other->move(x() + 40, y() + 40);
    other->show();
...
void ImageViewer::imgMomentsAction(){
    ...
    ImageViewer* other = new ImageViewer;
    other->loadImage(matImage, ImageConvert::cvMatToQImage(drawing), "image moments result");
    other->move(x() + 40, y() + 40);
    other->show();
}
...
void ImageViewer::harrisAction(){
    ...
    ImageViewer* other = new ImageViewer;
    other->loadImage(matImage, ImageConvert::cvMatToQImage(dst_norm_scaled), "corner harris result");
    other->move(x() + 40, y() + 40);
    other->show();
...
void ImageViewer::shiTomasiAction(){
    ...
    ImageViewer* other = new ImageViewer;
    other->loadImage(matImage, ImageConvert::cvMatToQImage(copy), "corner Shi-Tomasi result");
    other->move(x() + 40, y() + 40);
    other->show();
...
void ImageViewer::cornerDetectAction(){
    ...
    cv::cornerEigenValsAndVecs( src_gray, myHarris_dst, blockSize, apertureSize, cv::BORDER_DEFAULT );
    ...
    cv::cornerMinEigenVal( src_gray, myShiTomasi_dst, blockSize, apertureSize, cv::BORDER_DEFAULT );
    ...
    ImageViewer* other = new ImageViewer;
    other->loadImage(matImage, ImageConvert::cvMatToQImage(myShiTomasi_copy), "corner detector result");
    other->move(x() + 40, y() + 40);
    other->show();
}
...
void ImageViewer::featureSurfAction(int minHessian, bool refresh){
    ...
    //-- Draw keypoints
    cv::Mat img_keypoints;
    cv::drawKeypoints( matImage, keypoints, img_keypoints, cv::Scalar::all(-1), cv::DrawMatchesFlags::DEFAULT );

    if (refresh) {
        qImage = ImageConvert::cvMatToQImage(img_keypoints);
        imageLabel->setPixmap(QPixmap::fromImage(qImage));
        statusBar()->showMessage(tr("Min hessian: ") + QString::number(minHessian));
    }else{
        ImageViewer* other = new ImageViewer;
        other->loadImage(matImage, ImageConvert::cvMatToQImage(img_keypoints), "SUFT feature detector result");
        other->parameterPanel = new ui::SurfPanel(other);
        other->move(x() + 40, y() + 40);
        other->statusBar()->showMessage(tr("Min hessian: ") + QString::number(minHessian));
        other->show();
...
void ImageViewer::featureSiftAction(){
    ...
    ImageViewer* other = new ImageViewer;
    other->loadImage(matImage, ImageConvert::cvMatToQImage(img_keypoints), "SIFT feature detector result");
    other->move(x() + 40, y() + 40);
    other->show();

}

void ImageViewer::featureSiftActionWithMask(){
    qDebug() << "ImageViewer::featureSiftAction with a mask ";
    ...
    ObjectRemoval objRemover;
    Mat insect_mask, rectangle_mask;

    rectangle_mask = objRemover.identifyMask(matImage);
    cv::bitwise_not(rectangle_mask,insect_mask);

    detector.detect( src_gray, keypoints_mask, insect_mask);

    //-- Draw keypoints
    cv::Mat img_keypoints_mask;
    cv::drawKeypoints( src_gray, keypoints_mask, img_keypoints_mask, cv::Scalar::all(-1), cv::DrawMatchesFlags::DEFAULT );

    ImageViewer* other1 = new ImageViewer;
    other1->loadImage(matImage, ImageConvert::cvMatToQImage(img_keypoints_mask), "SIFT feature detector with mask result");
    other1->move(x() + 40, y() + 40);
    other1->show();
}
...
void ImageViewer::matchingBFAction(){
    ...
    ImageViewer* other = new ImageViewer;
    other->loadImage(matImage, ImageConvert::cvMatToQImage(img_matches), "description with brute force matcher");
    other->move(x() + 40, y() + 40);
    other->show();
}
void ImageViewer::matchingFLANNAction(){
    ...
    ImageViewer* other = new ImageViewer;
    other->loadImage(matImage, ImageConvert::cvMatToQImage(img_matches), "description with FLANN matcher");
    other->move(x() + 40, y() + 40);
    other->show();
}
void ImageViewer::matchingHomographyAction(){
    ...
    cv::Mat img_object = cv::imread( fileName.toStdString(), CV_LOAD_IMAGE_GRAYSCALE );
    ...
    ImageViewer* other = new ImageViewer;
    other->loadImage(matImage, ImageConvert::cvMatToQImage(img_matches), "Matching Homography result");
    other->move(x() + 40, y() + 40);
    other->show();
}
void ImageViewer::matchingTemplateAction(){
    ...
    ImageViewer* other = new ImageViewer;
    other->loadImage(matImage, ImageConvert::cvMatToQImage(img_display), "Matching template result");
    other->move(x() + 40, y() + 40);
    other->show();
}
...
void ImageViewer::pyramidAction(){
    ...
    ImageViewer* other = new ImageViewer;
    other->loadImage(matImage, ImageConvert::cvMatToQImage(dst), "Pyrymid result");
    other->move(x() + 40, y() + 40);
    other->show();
...
void ImageViewer::remapAction(){
        ...
        for (int i = 0; i < matImage.cols; i++) {
            ...
            case 0:
                if (i > matImage.cols * 0.25 && i < matImage.cols * 0.75
                        && j > matImage.rows * 0.25 && j < matImage.rows * 0.75) {
                    map_x.at<float>(j, i) = 2 * (i - matImage.cols * 0.25) + 0.5;
                    map_y.at<float>(j, i) = 2 * (j - matImage.rows * 0.25) + 0.5;
                } else {
    ...
    cv::remap( matImage, dst, map_x, map_y, CV_INTER_LINEAR, cv::BORDER_CONSTANT, cv::Scalar(0,0, 0) );

    ImageViewer* other = new ImageViewer;
    other->loadImage(matImage, ImageConvert::cvMatToQImage(dst), "Remapping result");
    other->move(x() + 40, y() + 40);
    other->show();
...
void ImageViewer::affineAction(){
    ...
    ImageViewer* other = new ImageViewer;
    other->loadImage(matImage, ImageConvert::cvMatToQImage(warp_rotate_dst), "Affine result");
    other->move(x() + 40, y() + 40);
    other->show();
...
void ImageViewer::equalizationAction(){
    ...
    ImageViewer* other = new ImageViewer;
    other->loadImage(matImage, ImageConvert::cvMatToQImage(dst), "Equalization result");
    other->move(x() + 40, y() + 40);
    other->show();
...
void ImageViewer::backprojectionAction(){
    ...
    ImageViewer* other = new ImageViewer;
    other->loadImage(matImage, ImageConvert::cvMatToQImage(backproj), "back projection result");
    other->move(x() + 40, y() + 40);
    other->show();
}
...
void ImageViewer::grayscaleMyImplAction(){
    ...
    ImageViewer* other = new ImageViewer;
    other->loadImage(ImageConvert::QImageToCvMat(img), img, "gray scale result");
    other->move(x() + 40, y() + 40);
    other->show();
}
...
// remove rectangle
void ImageViewer::objectRemovingAction(){
    qDebug() << "ImageViewer::objectRemovingAction ";

    ObjectRemoval obj;
    obj.removeObject(matImage);

    QImage img = ImageConvert::cvMatToQImage(matImage);

    ImageViewer* other = new ImageViewer;
    other->loadImage(matImage, img, "object removal");
    other->move(x() + 40, y() + 40);
    other->show();
}
